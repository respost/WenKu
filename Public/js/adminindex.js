...
jQuery(function(){
    ...
    $('#J_lmenu').on('click','.J_switchs', function(i){
    ...
    //常用功能
    $('#J_often_menu').on('click', function(){
        var title = $(this).attr('title');
        var data_uri = $(this).attr('data-uri');
        $.dialog({id:'often_menu', title:title, padding:'', lock:true});
        var dialog = $.dialog.get('often_menu');
        $.get(data_uri, function(html){
            dialog.content(html);
        });
        $('#often_menu a').on('click', function(){
            dialog.close();
        });
    });
    ...
    //关闭TAB
    $('#J_mtab_h').on('click','a.del', function(){
