...
class loadTag {
    ...
    public function js($options) {
        ...
        $statics_url = C('mtceo_statics_url') ? C('mtceo_statics_url') : './public';
        if (!is_file($path)) {
            //静态资源地址
        ...
        echo ( '<script type="text/javascript" src="' . __ROOT__ . '/data/static/' . md5($options['href']) . '.js?' . MTCEO_RELEASE . '"></script>');
    }
}