...
class AvatarSet
{
// 例如http://127.0.0.1/mtceo/qwef.php
	...
	//http://127.0.0.1/mtceo/app/lib/例如
	...
	// 例如D:\phpweb\mtceo\app\Lib\ORG\包含这个php文件的本地文件夹，区分大小写（尾部有一个 DIRECTORY_SEPARATOR 也就是\）
   ...
   //得到头像存储的url，例如HTTP://
	...
	public function renderHtml( $uid )
	{
		...
		$path=C('mtceo_attach_path');
        $input = urlencode("uid={$uid}&path={$path}");
		$baseUrl = $this->getswfurl();
		//$uc_api =C('mtceo_attach_path');
