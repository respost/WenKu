...
class alter_scoreBehavior extends Behavior {
    ...
    private function _alter_score($_data) {
        $score = C('mtceo_score_rule.'.$_data['action']); //获取积分变量
    ...
    private function _check_num($uid, $action){
        ...
        //登陆次数限制
        $max_num = C('mtceo_score_rule.'.$action.'_nums');
        //先检查统计信息
        ...
        $user_stat_mod->where(array('uid'=>$uid, 'action'=>$action))->setField('num', $new_num);
        return $return;
    }
